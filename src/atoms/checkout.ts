...
import { Coupon, DeliveryAddress, PaymentOption, Product } from "../types/jamma"
...
import { calculateTotalPrice, countCart, getCheckedProducts, isAllChecked } from "../utils/cart"
...
export const defaultAddressAtom = selector<DeliveryAddress | null>({
  ...
  get: async ({get}) => {
    ...
    if (deliveryAddresses.length > 0) {
      const defaultAddress = deliveryAddresses.find(address => address._default)
      if (defaultAddress) {
...
export const checkoutItemsAtom = selector<cartCheckedProductsAtom>({
  ...
  get: ({get}) => {
    ...
    const {totalPrice, totalPriceFormatted} = calculateTotalPrice(products, cart);
    return {
      checkoutProducts,
...
export const checkoutPricesAtom = selector({
  key: 'checkoutPrices',
  get: ({get}) => {
    const {totalPrice: merchandise_subtotal, totalCount} = get(checkoutItemsAtom)
...
export const selectedVoucherAtom = atom<Coupon | null>({
  ...
  effects_UNSTABLE: [
    ...
    ({ onSet }) => onSet((newVoucher) => console.log("applied voucher", newVoucher))
  ]
})
...
export const noteAtom = atom<string | null>({
  ...
  default: null,
  effects:[
    ({onSet}) => onSet((newMessage) => console.log("delivery message: \n", newMessage))
  ]
})
...
export const paymentOptionAtom = atom<PaymentOption | null>({
  ...
  effects: [
    ({ onSet }) => onSet((newPaymentOption) => log("New Payment Option", newPaymentOption))
  ]
})
