...
import { Coupon, DeliveryAddress, DeliveryStatus, OrderStatus, OrderType, PaymentOption, PaymentStatus, Product } from "../types/jamma";
import { getCoupon, getOrderStatus, getPaymentOption, getPaymentStatus, getOrderedProducts, getDeliveryAddress } from "../utils/order";
...
export const orderedProducts = selector<Product[] | null>({
  ...
  get: async ({get}) => {
    ...
    const {data} = await getOrderedProducts(order.products.products.map((p) => p.product_id));
    return data;
  }
})
...
export const orderAddress = selector<DeliveryAddress | null>({
  ...
  get: async ({get}) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const {data, error} = await getDeliveryAddress(order.delivery_address);
    return data;
  }
})
...
export const paymentOption = selector<PaymentOption | null>({
  ...
  get: async ({get}) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const {data, error} = await getPaymentOption(order.payment_option);
    return data;
  }
})
...
export const paymentStatus = selector<PaymentStatus | null>({
  ...
  get: async ({get}) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const {data, error} = await getPaymentStatus(order.payment_status);
    return data;
  }
})
...
export const voucher = selector<Coupon | null>({
  ...
  get: async ({get}) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const {data, error} = await getCoupon(order.voucher);
    return data;
  }
}) 