  ...
  return (
    <Box>
      ...
      >
        ...
        <Stack width="100%">
          <CardBody>
            <Heading size="sm">{phpString.format(voucher.discount)}</Heading>
            <Text marginBottom={3}>
        ...
        <Box display="flex" padding={5} alignItems={"center"}>
          {totalPrice > voucher.min_purchase_amount && !props.selected && (
            <Button size="sm" colorScheme={"green"} onClick={handleChange}>
              Claim
            </Button>
          )}
        </Box>
